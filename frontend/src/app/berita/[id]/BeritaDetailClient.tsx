...
import { ArrowLeft, Calendar, User, FileText, ImageIcon } from "lucide-react";
import Breadcrumb from "@/components/Breadcrumb";
import PdfViewer from "@/components/PdfViewer";
import { fallbackNews } from "@/lib/data";
import type { News, Attachment } from "@/types";
    ...
    return (
      <div className="min-h-screen bg-[#F7F7F7] pt-24">
        <div className="max-w-4xl mx-auto px-4 md:px-8 py-12 text-center">
          <h1 className="font-serif text-2xl font-bold text-[#1A1A1A] mb-4">Berita tidak ditemukan</h1>
          <Link href="/berita" className="text-[#A42A28] font-sans text-sm">Kembali ke daftar berita</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#F7F7F7]">
      <div className="bg-[#1A1A1A] py-16 md:py-24">
        <div className="max-w-4xl mx-auto px-4 md:px-8">
          <Link href="/berita" className="inline-flex items-center gap-2 text-white/60 hover:text-white font-sans text-sm mb-6 transition-colors">
            <ArrowLeft size={16} /> Kembali ke Berita
          </Link>
          <span className="inline-block bg-[#A42A28] text-white text-xs font-sans px-3 py-1 mb-4">{item.category}</span>
          <h1 className="font-serif text-2xl md:text-4xl font-bold text-white mb-4 leading-tight">{item.title}</h1>
      ...
      <Breadcrumb items={[{ label: "Berita", to: "/berita" }, { label: item.title }]} />
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-8 py-12">
        ...
        <div className="bg-white border border-[#E5E5E5] p-6 md:p-10 mb-8">
          <p className="font-serif text-lg text-[#1A1A1A] mb-6 leading-relaxed">{item.summary}</p>
          ...
          <div className="bg-white border border-[#E5E5E5] p-6">
            ...
            <div className="space-y-2">
              {pdfs.map((pdf: Attachment) => (
                <button key={pdf.id} onClick={() => setPdfUrl(pdf.dataUrl)}
                ...
                >
                  ...
                  <span className="font-sans text-sm text-[#1A1A1A] flex-1 truncate">{pdf.name}</span>
                  <span className="font-sans text-xs text-[#999999]">{(pdf.size / 1024).toFixed(0)} KB</span>
                </button>
              ))}
            </div>
          </div>
